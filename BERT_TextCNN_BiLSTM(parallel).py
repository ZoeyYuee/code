...
class BERT_TextCNN_BiLSTM(nn.Module):
    ...
    # TextCNN
        cnn_out1 = self.conv_and_pool(self.conv1, bert_out.unsqueeze(1))  # (batch_size, hidden_dim)
        cnn_out2 = self.conv_and_pool(self.conv2, bert_out.unsqueeze(1))
        cnn_out3 = self.conv_and_pool(self.conv3, bert_out.unsqueeze(1))
        cnn_out = torch.cat([cnn_out1, cnn_out2, cnn_out3], dim=1)  # (batch_size, hidden_dim*3)

    # BiLSTM
        lstm_out, (hidden_last, cn_last) = self.lstm(bert_out, hidden)  # lstm_out: (batch_size, max_len, hidden_dim*2)

    # Concatenate TextCNN and BiLSTM outputs
        all_out = torch.cat([cnn_out, lstm_out[:, -1, :]], dim=-1)  # (batch_size, hidden_dim*3 + hidden_dim*2)
