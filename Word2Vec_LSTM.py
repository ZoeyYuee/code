...
def predict_test_data(user):
    ...
    checkpoint = ModelCheckpoint('lstm_model.h5', monitor='val_accuracy', mode='max', save_best_only=True, verbose=1)

    model.fit(X_train, labels_train, validation_split=0.2, epochs=1, batch_size=32, callbacks=[checkpoint])
